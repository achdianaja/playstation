...
include "../../connection.php";
    ...
    if (move_uploaded_file($_FILES['new_photo']['tmp_name'], $folder . $_FILES['new_photo']['name'])) {
        ...
        if ($upload) {
            ...
            echo "<script> alert('CHANGE SUCCESSED!');window.location.replace('../../views/customer/profile.php');</script>";
        } else {
            echo "<script> alert('CHANGE FAILED!');window.location.replace('user_photo.php?id='$_POST[user_id]');</script>";
        }
        //upload failed
    } else {
        echo "<script> alert('UPLOAD FAILED!');window.location.replace('user_photo.php?id='$_POST[user_id]');</script>";
    }
}
